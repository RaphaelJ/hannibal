...
module Hannibal.Filesystem.FileDesc (
    Digest (..),
    FileDesc (..),
    PieceDesc (..),
    pieceSize,
...
newtype Digest = Digest {dValue :: ByteString}
...
fileDesc content =
    let pieces = map pieceDesc $ splitFile content
        fileDigest = digest $ LBS.fromChunks $ map (dValue . pdDigest) pieces
